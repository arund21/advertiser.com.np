...
function advertisements($size) {
    $ad = \App\Models\Advertise::where('resolution', $size)->where('status',1)->inRandomOrder()->first();
    if (!empty($ad)) {
        if ($ad->type == 1) {
            return  '<a  target="_blank" href="'.$ad->redirect_url.'" class="clickUp" data-id="'.$ad->id.'"><img src="'.getImage('assets/images/advertisement/'.$ad->ad_image).'" alt="image" class="w-100"></a>';
        }
        if($ad->type == 2) {
            return $ad->script;
        }
